...
class Login(object):
    ...
    def save_entry_db(self, username, password, name, cash):
        # Insert entry
        self.cursor.execute('''INSERT INTO users(username, password, first_name, cash) VALUES(?,?,?,?)''', (username, password, name, cash))
        self.db.commit()

    # Reads the database, gets logged in user
    def login_create_user(self, user_name, password):
        ...
        try:
            ...
            if password == user_db[2]:
                shopper = Shopper()
                shopper.create_user(user_db[3], user_db[1], user_db[2], user_db[4])
            else:
                self.clear()
    ...
    def user_login(self):
        ...
        self.login_create_user(email, password)
